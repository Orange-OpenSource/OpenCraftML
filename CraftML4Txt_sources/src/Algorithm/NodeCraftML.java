...
public class NodeCraftML implements GenericNodeCraftML {
			...
			for(int i = 0;i<realNbChildren;i++) {
				...
				if(newRecord.length!=12) {
					...
					if(newRecord[6].equals("none")){
						//TODO ce cas n'arrive jamais car les centroids s�parateur ne sont jamais nuls
	...
	public void storeNode(RecordTextWriter file,float[] instanceValues,int[] instanceIndexes) {
		 ...
		 * champs 6 = centro�d qui amene � ce noeud. sep ; et =
		 * champs 7 = nombre d'instances
		 * champs 8 � 11 = graines (2 graines pour x : une pour l'index et une pour le signe, idem pour y)
		*/
	...
	public void computesSeparativeInstancesInt(ArrayList<int[]> keyX,ArrayList<float[]> valuesX,ArrayList<Integer> useInst,ArrayList<String[]> keyY,ArrayList<float[]> valuesY){
		...
		float[] globalNormCols = new float[dimXProj];
		float val;
		for(int i=0;i<branchFactor;i++){
			if(nbInst[i]!=0){
				for(int j = 0;j<dimXProj;j++){
					val = childrenInstancesCurrent[i][j]/(float) nbInst[i];
					globalNormCols[j] = globalNormCols[j]+val*val;
				}
			}
		}
		...
		for(int i=0;i<branchFactor;i++){
			if(nbInst[i]!=0){
				for(int j = 0;j<dimXProj;j++){
					if(globalNormCols[j] != 0) {
						childrenInstancesCurrent[i][j] = childrenInstancesCurrent[i][j]/(float) Math.sqrt(globalNormCols[j]);
					}
				}
			}
		}
