 ...
 *
 * 
 * Important: le bool�en optimizeMemory d�finit si on utilise le mode craftml standard qui charge le dataset en m�moire (false) ou non (true).
 * 
...
public class CraftML {
	...
	public void loadModel(String loadPath, String modelName){
		...
		boolean ok=loadModelParameters(loadPath + modelName + "_parameters.txt");
		if (!ok) {
			System.out.println("model: "+modelName+" for path: "+loadPath+": parameters not acessible");
			hasLearnt=false;
			return ;
		}
	...
	public boolean loadModelParameters(String fileName){
		
		RecordTextReader mRecordTextReader = new RecordTextReader();
		boolean ok=mRecordTextReader.openFile(fileName);
		if (!ok) {
